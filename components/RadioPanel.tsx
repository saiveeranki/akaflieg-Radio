...
export const RadioPanel: React.FC<RadioPanelProps> = ({ 
  ...
  const voices = ['Charon', 'Kore', 'Puck', 'Fenrir', 'Zephyr'];
  ...
  return (
    <div className="bg-slate-900 border-4 border-slate-700 rounded-xl p-5 radio-glow max-w-md w-full mx-auto shadow-2xl">
      <div className="bg-black p-4 rounded-lg mb-6 border-2 border-slate-800 flex justify-between items-center relative overflow-hidden">
        ...
        <button onClick={handleFlip} className="mx-4 p-2 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-full border border-slate-600 transition-transform active:rotate-180">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label className="text-slate-500 text-[9px] font-black uppercase block px-1 mb-2 tracking-widest">Presets</label>
            <div className="grid grid-cols-1 gap-1">
              {Object.entries(RadioService).map(([key, val]) => (
                <button
                  key={key}
                  onClick={() => setStandbyFrequency(val as string)}
          ...
          <div>
            <label className="text-slate-500 text-[9px] font-black uppercase block px-1 mb-2 tracking-widest">ATC Voice</label>
            <select 
              value={selectedVoice} 
              onChange={(e) => onVoiceChange?.(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 text-slate-300 rounded px-2 py-1.5 text-[10px] font-bold outline-none focus:border-amber-500"
            >
        ...
        <div className="flex flex-col items-center justify-center bg-slate-800/50 rounded-lg border border-slate-700/50 p-4 relative">
          <div className="absolute left-2 top-4 bottom-4 w-1 bg-slate-900 rounded-full overflow-hidden">
            <div 
              className={`w-full absolute bottom-0 transition-all duration-100 ${isTransmitting ? 'bg-amber-500' : 'bg-slate-700'}`}
          ...
          <button
            ...
            className={`w-28 h-28 rounded-full border-4 flex flex-col items-center justify-center transition-all transform active:scale-95 select-none ${
              isTransmitting 
          ...
          >
            <span className={`text-[10px] font-black mb-1 tracking-tighter ${isTransmitting ? 'text-white' : 'text-slate-500'}`}>TRANSMIT</span>
            <svg className={`w-10 h-10 ${isTransmitting ? 'text-white' : 'text-slate-500'}`} fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/>
            </svg>
            <span className={`text-[8px] font-bold mt-1 ${isTransmitting ? 'text-red-200' : 'text-slate-600'}`}>(PTT)</span>
          </button>
          
          <div className="mt-4 flex flex-col items-center">
             <span className="text-[8px] text-slate-500 font-black uppercase tracking-[0.2em]">Voice Input</span>
          </div>
        </div>
      </div>
    </div>
